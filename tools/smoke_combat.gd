...
func _test_grunt_kill_chain() -> void:
	...
	# Grunt default is now health=2 for perceptibility; this test checks the died-payload seam.
	e.health = 1
	root.add_child(e)
...
func _test_weapon_kill_seam() -> void:
	...
	# hit_confirmed fires immediately; kill_confirmed fires when e.died emits (pinned health=1, fatal).
