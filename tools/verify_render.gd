extends SceneTree
# Layer 3 of godot-verify: boots a scene, renders ~20 frames, and fails if the
# output is a flat color (the signature of "valid scene, renders nothing" —
# e.g. a camera pointing away from the world, missing sky, no current camera).
#
...
func _process(_delta: float) -> bool:
	...
	if spread < FLAT_SPREAD:
		_fail("renders a flat color (avg luminance %.3f, spread %.3f) — nothing visible; check camera direction/current, lights, sky" % [avg, spread])
	else:
		print("VERIFY-RENDER: OK — %s (avg luminance %.3f, spread %.3f)" % [_scene_path, avg, spread])
		quit(0)
	return true
