...
func _check_text_properties(path: String, root: Node) -> void:
	...
	for raw_line in FileAccess.get_file_as_string(path).split("\n"):
		...
		if line.begins_with("[node"):
			...
			current = root if node_path == "." else root.get_node_or_null(node_path)
			...
			if prop != "" and not _has_property(current, prop):
				_fail(path, current_label, "unknown property \"%s\" (silently dropped by Godot)" % prop)
