...
Given an art need, produce the **classification** (below) plus a **tailored generation prompt** and a **recommended generator**. The orchestrator files this as the `Asset: <name>` task note. Do not invent a hardcoded prompt — write one specific to *this* asset, building on the patterns in `library/asset-sources.md`.
...
- **Location** — the file is under `assets/textures/`, not stray in `assets/` or elsewhere. A stray file is a FAIL whose fix is a godot-dev *move* task (it updates any `.tscn`/`.import` references).
