...
If any check fails, do not dispatch. Report in one line *which* check failed and why, then offer the right route: **game-designer** (too big / unclear scope), **skill-researcher** (uncovered pattern — check 1 failed). Failing the check is a correct outcome of /quick, not an error.
