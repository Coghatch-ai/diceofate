...
- **Seamless**: all randomness goes through `_periodic_hash(x, y, size, seed)`, which wraps `x`/`y` mod `size` *before* hashing, so the value at `x` equals the value at `x+size`. Any repeating period (band/thread/cell) must divide `size` evenly. Never sample raw noise that isn't periodic.
