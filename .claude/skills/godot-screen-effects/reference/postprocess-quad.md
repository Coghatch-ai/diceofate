...
- An active `Camera3D` in the scene. If the project uses the SubViewport pixelation setup (skill `godot-3d-pixelation`), the camera — and therefore this quad — lives **inside the SubViewport**, so the effect is applied *before* upscaling. That is the correct order for pixel-art outlines.
...
| Works in game but not in editor viewport | Quad is not a child of the *editor-previewed* camera; check the camera preview toggle |
