...
6. **Stylized-flat bias (keep the palette dominant):** push **roughness high**, **metallic low**, **normals mild** (`normal_scale` down). Map sourced albedo toward the nearest `tools/art_style.gd` swatch family (concrete → CONCRETE_*, steel → STEEL_*, rust → RUST_*, wood → WOOD_*); keep within `SATURATION_CEILING = 0.50`, value `[0.16, 0.90]`. When a sourced albedo is louder/glossier than the palette, dial roughness up / metal down — don't admit a new bright hue. NOT photoreal, NOT flat-toon.
