...
> **Rebuild note (2026-06-14).** `levels/shared_apartment.tscn` no longer exists on disk (only the Godot editor cache remains); the builder `tools/build_shared_apartment.gd` and MeshLibrary `resources/apartment_tiles.meshlib.tres` survive and are committed. The grid `levels/drawn/current.json` is unchanged — all cell coordinates below are still valid. This slice is regenerated by **re-running the builder** (`$GODOT --headless --path . --script tools/build_shared_apartment.gd`), which already emits the shell AND the Slice-2 master-bedroom props in one pass. See `design/shared-apartment-rebuild.md` for the consolidated dispatch order.
