...
Drawing levels (blockout from a sketch): the web UI's **Draw level** tool (🗺) lets the user paint a small top-down tile grid — wall / door / window / item — that exports to `levels/drawn/current.json` (tile codes 0 floor, 1 wall, 2 door, 3 window, 4–7 item types by colour). On export the orchestrator spawns **level-designer** (Sonnet): it reads the grid, interviews the user for the level **name** and every **scene** detail (metres per cell — fixing the "came out like metres" scale trap — wall height, what door/window/item become, player spawn, theme) via `mcp__ui__form`, writes a short brief to `design/levels/<name>.md`, then hands off to **godot-dev** to build a **named** level scene (`levels/<name>.tscn`, root node `<Name>`) using the reusable **guided-level** builder (`levels/guided_level.gd`, `class_name GuidedLevel` — "guided" names the build mechanism, geometry guided by the drawn grid, NOT the level itself) reading the saved grid `levels/drawn/<name>.json`, **merging contiguous wall runs** (never one StaticBody per cell), auto-spawning the Player, registered in `main.gd` → `godot-verify`. The builder is reusable across levels; only the named scene + its grid are per-level. Prototype path: "for an idea", not a finished level. The grid resolution + canvas live in the framework UI (`xenodot-forge/ui/client/draw-level.js`); only the in-game builder is godot-dev's.
